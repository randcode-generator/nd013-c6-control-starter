...
int main ()
{
  ...
  **/
  PID pid_steer = PID();
  pid_steer.Init(0.23, 0.002, 0.15, -1.2, 1.2);
  ...
  **/
  ...
  pid_throttle.Init(0.23, 0.00005, 0.05, -1, 1);
  ...
  h.onMessage([&pid_steer, &pid_throttle, &new_delta_time, &timer, &prev_timer, &i, &prev_timer](uWS::WebSocket<uWS::SERVER> ws, char *data, size_t length, uWS::OpCode opCode)
  {
        ...
        if (s != "") {
          ...
          **/
          // The cross track error (CTE) is calculated by the first
          // y-axis waypoint minus car's current y-axis position
          double CTE = y_points[0] - y_position;
          error_steer = CTE;
          ...
          **/
          ...
          // If the CTE is greater then 2
          // stop the simulator
          if(CTE >= 2 || CTE <= -2) {
            // Set steering output to nan
            steer_output = std::nan("1");
          }
          ...
          **/
          ...
          // The error for throttle is the expect velocity minus
          // current velocity
